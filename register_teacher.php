...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
  ...
  if (isset($_POST['name']) && isset($_POST['num']) && isset($_POST['select']) && isset($_POST['email']) && isset($_POST['pass'])) {
    ...
    if (!(strlen($num) == 10)) {
      # code...
      $all_ok = 0;
      $msg = "NUMÉRO DE PORTABLE INVALIDE.";
    }

    if ($all_ok == 1) {
      # code...
      $insert = "insert into teacher(name,phone,email,password,status) values('$name',$num,'$email','$pass',0)";
...
<center>
  ...
  <form method="post" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']); ?>">
    <input type="text" placeholder="Nom: [A-Z]" name="Nom" autocomplete="Désactivé" required="Oui" pattern="[A-Za-z\\s]*" />
    ...
    <input type="password" placeholder="Mot de passe" name="Mot de passe" autocomplete="Désactivé" required="OUI" />
